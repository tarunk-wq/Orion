...
public class SearchUtils {
    ...
    public static List<DiscoveryConfiguration> getAllDiscoveryConfigurations(Context context, Item item)
            throws SQLException {
    	List<DSpaceObject> parents = new ArrayList<DSpaceObject>();
        parents.addAll(item.getCommunities());
        parents.addAll(item.getCollections());
        return getAllDiscoveryConfigurations(context, null, parents, item); 
    }
    ...
    public static List<DiscoveryConfiguration> getAllDiscoveryConfigurations(final Context context,
    	...
    	List<DSpaceObject> parents = new ArrayList<DSpaceObject>();
    	if (witem.getCommunity() != null) {       	
    		parents.add(witem.getCommunity());
        	
        } else {
        	parents.add(witem.getCollection());
        }
        return getAllDiscoveryConfigurations(context, "workspace", parents, witem.getItem());
    }
    ...
    public static List<DiscoveryConfiguration> getAllDiscoveryConfigurations(final Context context,
        ...
        return getAllDiscoveryConfigurations(context, "workflow", parents, witem.getItem());
    }

    private static List<DiscoveryConfiguration> getAllDiscoveryConfigurations(final Context context,
                                                                              String prefix,
                                                                              List<DSpaceObject> parents, Item item)
        throws SQLException {
        Set<DiscoveryConfiguration> result = new HashSet<>();

        for (DSpaceObject parent : parents) {
            addDiscoveryConfigurationForParents(context, result, prefix, parent);
        }
