...
public abstract class InprogressSubmissionIndexFactoryImpl
    ...
    @Autowired
    protected CommunityIndexFactory indexableCommunityService;
    @Autowired
    ...
    public void storeInprogressItemFields(Context context, SolrInputDocument doc,
        ...
        if (inProgressSubmission.getCollection() != null) {
        	locations = indexableCollectionService.getCollectionLocations(context, inProgressSubmission.getCollection());
        	locations.add("l" + inProgressSubmission.getCollection().getID());
        
        } else if (inProgressSubmission.getCommunity() != null) {
        	locations = indexableCommunityService.getCommunityLocations(context, inProgressSubmission.getCommunity());
        	locations.add("m" + inProgressSubmission.getCommunity().getID());
        
        } else {
        	throw new RuntimeException("inProgressSubmission dont have parent: " + inProgressSubmission.getID());
        }
