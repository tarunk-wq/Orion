...
public class PasswordAuthentication
    ...
    public int authenticate(Context context,
        ...
        if (username != null && password != null) {
            ...
            int unsuccessfulAttemptCount = DSpaceServicesFactory.getInstance().getConfigurationService()
                    .getIntProperty("unsuccessful.login.attempt.count");
            
            int deactivationResetTime = DSpaceServicesFactory.getInstance().getConfigurationService()
            		.getIntProperty("deactivation.reset.time");
            ...
            if (!eperson.canLogIn()) {
           	 long deactivationTime = getDeactivationTime(context,eperson);
           	 
           	 if(deactivationTime >= deactivationResetTime) {
         		context.turnOffAuthorisationSystem();
	        	try {
		        	eperson.setCanLogIn(true);
		        	eperson.setLastWrongAttemptTime(null);
		        	eperson.setUnsuccessfulAttempts(0);
					EPersonServiceFactory.getInstance().getEPersonService().update(context, eperson);
				} catch (SQLException | AuthorizeException e) {
					log.error(LogHelper.getHeader(context, "authenticate", "Error updating login attempt count for user" + 
							username), e);
				} finally {
            	    context.restoreAuthSystemState();
            	}
           	 }else {            		 
           		 //user exhausted maximum number of unsuccessful login attempts
           		 if(eperson.getUnsuccessfulAttempts() >= unsuccessfulAttemptCount) {
           			 log.warn(LogHelper.getHeader(context, "authenticate",
           					 "rejecting PasswordAuthentication because " + username + " has exhausted " +
           					 "maximum unsuccessful Login attempts."));
        			 String timeRemaining = "";
        			 long time = deactivationResetTime - deactivationTime;
        			 if(time > 60) {
        				 long hour = time / 60; 
        				 long minutes = time % 60;
        				 timeRemaining += (hour > 0 ? hour + " hour " : "") + (minutes > 0 ? minutes + " minutes " : ""); 
        			 }else {
        				 timeRemaining += time + " Minutes";
        			 }
        			 request.setAttribute("remainingTime", timeRemaining);
           			 return LOCK_OUT;
           		 }
            ...
            if (eperson.getRequireCertificate()) {
            ...
            } else if (eperson.getPasswordExpiryDate() != null && eperson.getPasswordExpiryDate().before(new Date())) {
            	log.warn(LogHelper.getHeader(context, "authenticate", "rejecting PasswordAuthentication because password for " 
            	...
            	//user exhausted maximum number of unsuccessful login attempts
            	int unsuccessfulAttempts = eperson.getUnsuccessfulAttempts() + 1;
            	context.turnOffAuthorisationSystem();
            	try {
            		if(unsuccessfulAttempts >= unsuccessfulAttemptCount) {
                		eperson.setCanLogIn(false);
                	}
                	eperson.setUnsuccessfulAttempts(unsuccessfulAttempts);
                	eperson.setLastWrongAttemptTime(Instant.now());
					EPersonServiceFactory.getInstance().getEPersonService().update(context, eperson);
				} catch (SQLException | AuthorizeException e) {
					log.error(LogHelper.getHeader(context, "authenticate", "Error updating login attempt count for user" + 
							username), e);
            	...
            	if(unsuccessfulAttempts >= unsuccessfulAttemptCount) {
            		request.setAttribute("remainingTime", deactivationResetTime + " Minutes");
            		log.warn(LogHelper.getHeader(context, "authenticate",
                            "Locking user with username - " + username + " - exhausted maximum"
                            		+ " number of unsuccessfull login attempts."));
            		return LOCK_OUT;
            	}
    ...
    private long getDeactivationTime(Context context, EPerson eperson) {
		long time = 0;
		try {
			Instant wrongAttemptTime = eperson.getLastWrongAttemptTime();
			time = calculateTimeIntervalDifference(Instant.now(), wrongAttemptTime);
		} catch (Exception e) {
			log.error("Error in calculating deactivation time",e);
		}
		return time;
	}
    
	private long calculateTimeIntervalDifference(Instant currentTime, Instant previousTime) {
