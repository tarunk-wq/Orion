...
public class DCInputsReader {
    ...
    /* For generating dynamic form for single submission */
    private List<Map<String,String>> createListOfFieldMap(Context context, HashMap<String,String> metadataMap, String collectionHandle){
    	List<Map<String,String>> listOfMap = new ArrayList<Map<String,String>>();
    	/* create dynamic form wrt the metadata */
    	for(String key : metadataMap.keySet()) {
    		UserMetadataFields userMetadatafield = null;
    		Integer fieldType = 0;
			try {
				userMetadatafield = userMetadataFieldsService.findUserMetadatafieldBySystemName(context,
						collectionHandle, key);
			} catch (Exception e) {
				log.error("Error in getting usermetadata field");
			}
			if(userMetadatafield != null) {
				fieldType = userMetadatafield.getFieldType();
			}
    		Map<String, String> fieldMap = new HashMap<String,String>();
    		String[] dc_element_qualifier = key.split("\\.");
    		// qualifier is not available
    		if(dc_element_qualifier.length == 2) {
    			fieldMap.put("dc-element", dc_element_qualifier[1]);
//    			fieldMap.put("dc-qualifier", "");
    		}else {
    			// qualifier available
    			fieldMap.put("dc-element", dc_element_qualifier[1]);
    			fieldMap.put("dc-qualifier", dc_element_qualifier[2]);
    		}
    		fieldMap.put("dc-schema", "dc");
    		fieldMap.put("label", metadataMap.get(key));
    		if(fieldType == 2) {
    			fieldMap.put("input-type", "date");
    		} else if(fieldType == 1) {
    			fieldMap.put("input-type", "integer");
    		} else {
    			fieldMap.put("input-type", "onebox");
    		}
    		fieldMap.put("hint", "Enter the "+ toTitleCase(metadataMap.get(key))+" of this item.");
    		fieldMap.put("isUniqueMetadata", String.valueOf(userMetadatafield.getIsUniqueMetadata()));
//    		DCInput field = new DCInput(fieldMap,null);
    ...
    List<Map<String,String>> sortAlphabetically(List<Map<String,String>> listOfMap){
    	List<Map<String, String>> sortedListOfMap;
    	
    	listOfMap.sort(Comparator.comparing(
                map -> map.get("label"),
    ...
    public DCInputSet getInputsByFormName(Context context, String formName)
            throws DCInputsReaderException, SQLException {
		// for adminFiles and caseFiles
		if(!formName.contains("123456789/")) {
			List<List<Map<String, String>>> pages = formDefns.get(formName);
	        if (pages == null) {
	            throw new DCInputsReaderException("Missing the " + formName + " form");
	        }
    	...
    	String collectionHandle = formName;
    	HashMap<String,String> metadataMap = new HashMap<>();
    	try {
    		metadataMap = userMetadataFieldsService.getMetadataFieldsBySubDeptHandle(context, collectionHandle);
    	}catch(Exception e) {
    		//error
    	}
    	List<Map<String,String>> listOfMap = new ArrayList<Map<String,String>>();
    	if(metadataMap.size()>0) {
    		listOfMap = createListOfFieldMap(context, metadataMap, collectionHandle);
    	}else {
    		// if dynamic form for collection not available
    		Map<String,String> fieldMap = new HashMap<>();
    		fieldMap.put("dc-element", "title");
        	fieldMap.put("dc-qualifier", "null");
        	fieldMap.put("dc-schema", "dc");
        	fieldMap.put("label", "Dynamic Form for this Collection not available!");
        	listOfMap.add(fieldMap);
    	}
    	//sorting on the basis of metadata name to send a sorted list to FE
    	sortAlphabetically(listOfMap);
    	List<List<Map<String, String>>> rows = new ArrayList<List<Map<String,String>>>();
    	List<Map<String, String>> currentRow = new ArrayList<>();
    	for(Map<String,String> map : listOfMap) {
    		currentRow.add(map);
    		
    		if (currentRow.size() == 2) {  // row full
    	        rows.add(currentRow);
    	        currentRow = new ArrayList<>(); // start new row
    	    }
    	}
    	
    	// If number of maps is odd, add the last partial row
    	if (!currentRow.isEmpty()) {
    	    rows.add(currentRow);
    	}
        
        lastInputSet = new DCInputSet("dynamicForm",rows, valuePairs);
        return lastInputSet;
    }
