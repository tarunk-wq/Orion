...
public class ItemServiceImpl extends DSpaceObjectServiceImpl<Item> implements ItemService {
    ...
    public Item create(Context context, WorkspaceItem workspaceItem,
    	...
    	DSpaceObject parent = workspaceItem.getParent();
        authorizeService.authorizeAction(context, parent, Constants.ADD);
    ...
    @Override
    public Iterator<Item> findAllByCommunity(Context context, Community community) throws SQLException {
        return itemDAO.findAllByCommunity(context, community);
    }
...
//                    modified = true;
    ...
    protected void rawDelete(Context context, Item item) throws AuthorizeException, SQLException, IOException {
        ...
        //clear communities from item
        item.clearCommunities();
        item.setOwningCommunity(null);
    ...
    @Override
    public boolean isOwningCommunity(Item item, Community community) {
    	Community owningCommunity = item.getOwningCommunity();
    	
    	return owningCommunity != null && community.getID().equals(owningCommunity.getID());
    }
    ...
    @Override
    public void inheritCollectionDefaultPolicies(Context context, Item item, DSpaceObject parent,
                                                 boolean replaceReadRPWithCollectionRP)
        throws SQLException, AuthorizeException {

        adjustItemPolicies(context, item, parent, replaceReadRPWithCollectionRP);
        adjustBundleBitstreamPolicies(context, item, parent, replaceReadRPWithCollectionRP);
    ...
    @Override
    public void adjustBundleBitstreamPolicies(Context context, Item item, DSpaceObject parent, boolean replaceReadRPWithCollectionRP)
        throws SQLException, AuthorizeException {
        List<ResourcePolicy> defaultCollectionBundlePolicies = authorizeService
                .getPoliciesActionFilter(context, parent, Constants.DEFAULT_ITEM_READ);
        // Bitstreams should inherit from DEFAULT_BITSTREAM_READ
        
        List<ResourcePolicy> defaultCollectionBitstreamPolicies = authorizeService
            .getPoliciesActionFilter(context, parent, Constants.DEFAULT_BITSTREAM_READ);

        List<ResourcePolicy> defaultItemPolicies = authorizeService.findPoliciesByDSOAndType(context, item,
                ResourcePolicy.TYPE_INHERITED);
        if (defaultCollectionBitstreamPolicies.size() < 1) {
            throw new SQLException("Collection " + parent.getID()
                                       + " (" + parent.getHandle() + ")"
                                       + " has no default bitstream READ policies");
        }
...
//                addDefaultPoliciesNotInPlace(context, bitstream, defaultCollectionPolicies);
    ...
    @Override
    public void adjustItemPolicies(Context context, Item item, DSpaceObject parent, boolean replaceReadRPWithCollectionRP)
        throws SQLException, AuthorizeException {
        // read collection's default READ policies
//        List<ResourcePolicy> defaultCollectionPolicies = authorizeService
//            .getPoliciesActionFilter(context, collection, Constants.DEFAULT_ITEM_READ);
    	if(parent instanceof Collection) {
    		parent = collectionService.find(context, parent.getID());
    	} else if (parent instanceof Community) {
    		parent = communityService.find(context, parent.getID());
    	} else {
    		DSpaceObjectService<DSpaceObject> dsoService = ContentServiceFactory.getInstance().getDSpaceObjectService(parent);
    		parent = dsoService.find(context, parent.getID());
    	}
    	
        List<ResourcePolicy> defaultCollectionPolicies = parent.getResourcePolicies();
        ...
        // MUST have default policies
        if (defaultCollectionPolicies.size() < 1) {
            throw new SQLException("Item parent " + parent.getID()
                                       + " (" + parent.getHandle() + ")"
                                       + " has no default item READ policies");
        }
...
//            addDefaultPoliciesNotInPlace(context, item, defaultCollectionPolicies);
            addDefaultCollectionPoliciesNotInPlace(context, item, defaultCollectionPolicies);
        } finally {
            context.restoreAuthSystemState();
        }
    }
...
prevent the generation of resource policy entry values with null dspace_object as value
    ...
    */
    ...
    public DSpaceObject getAdminObject(Context context, Item item, int action) throws SQLException {
        ...
        DSpaceObject parent = getParentObject(context, item);
        ...
        switch (action) {
            case Constants.ADD:
                ...
                if (AuthorizeConfiguration.canItemAdminPerformBitstreamCreation()) {
                    adminObject = item;
                } else if (collection != null && AuthorizeConfiguration.canCollectionAdminPerformBitstreamCreation()) {
            ...
            case Constants.REMOVE:
                ...
                if (AuthorizeConfiguration.canItemAdminPerformBitstreamDeletion()) {
                    adminObject = item;
                } else if (collection != null && AuthorizeConfiguration.canCollectionAdminPerformBitstreamDeletion()) {
                    adminObject = collection;
            ...
            case Constants.WRITE:
                ...
                if (item.getOwningCollection() == null) {
                    if (collection != null && AuthorizeConfiguration.canCollectionAdminManageTemplateItem()) {
                        adminObject = collection;
    ...
    @Override
    public void moveToCommunity(Context context, Item item, Community from, Community to, boolean inheritDefaultPolicies) throws SQLException, AuthorizeException, IOException {
        // If the two communities are the same, do nothing.
        if (from != null && from.equals(to)) {
            return;
        }
        
        // Check authorisation on the item before the move
        if (!canEdit(context, item)) {
            authorizeService.authorizeAction(context, item, Constants.WRITE);
        }
        ...
        if (from != null) {
        	communityService.removeItem(context, from, item);
        }
        
        // Add to new community
        communityService.addItem(context, to, item);
        
        log.info(LogHelper.getHeader(context, "move_item_to_community",
                "item_id=" + item.getID() + ", from community_id=" +
                (from != null ? from.getID() : "null") + " to community_id=" + to.getID()));
        
        // Update owning community
        item.setOwningCommunity(to);
        
        // If applicable, update the item policies
        if (inheritDefaultPolicies) {
            log.info(LogHelper.getHeader(context, "move_item_to_community","Updating item with inherited policies"));
            inheritCollectionDefaultPolicies(context, item, null);
        }
    ...
    @Override
    public void moveItemCollectionToCommunity(Context context, Item item, Collection from, Community to, boolean inheritDefaultPolicies) throws SQLException, AuthorizeException, IOException {
        // If collection already in that community
        if (to != null && to.equals(item.getOwningCommunity())) {
            return;
        }
        ...
        if (from != null) {
        	collectionService.removeCollectionMapping(context, from, item);
        }
        
        // Add to new community
        communityService.addItem(context, to, item);
        
        log.info(LogHelper.getHeader(context, "move_item_collection_to_community",
                "item_id=" + item.getID() +
                ", from collection_id=" + (from != null ? from.getID() : "null") +
                " to community_id=" + to.getID()));
    			
        item.setOwningCollection(null); // no longer in a collection
        item.setOwningCommunity(to);
    	
        if (inheritDefaultPolicies) {
            log.info(LogHelper.getHeader(context, "move_item_collection_to_community","Updating item with inherited policies from community"));
            inheritCollectionDefaultPolicies(context, item, null);
        }
    ...
    @Override
    public void moveItemCommunityToCollection(Context context, Item item, Community from, Collection to, boolean inheritDefaultPolicies) throws SQLException, AuthorizeException, IOException {
        if (to != null && to.equals(item.getOwningCollection())) {
            return;
        }
        ...
        log.info(LogHelper.getHeader(context, "move_item_community_to_collection",
                "item_id=" + item.getID() +
                ", from community_id=" + (from != null ? from.getID() : "null") +
                " to collection_id=" + to.getID()));
    			
        item.setOwningCommunity(null); // no longer in a community
        item.setOwningCollection(to);
        
        if (inheritDefaultPolicies) {
            log.info(LogHelper.getHeader(context, "move_item_community_to_collection","Updating item with inherited collection policies"));
            inheritCollectionDefaultPolicies(context, item, to);
        }
    ...
    protected void addDefaultCollectionPoliciesNotInPlace(Context context, DSpaceObject dso,
		...
		for (int i = 0; i < defaultCollectionPolicies.size(); i++) {
			ResourcePolicy resourcePolicy = defaultCollectionPolicies.get(i);
			if (!authorizeService.isAnIdenticalPolicyAlreadyInPlace(context, dso, resourcePolicy.getGroup(),
					resourcePolicy.getAction(), resourcePolicy.getID())) {
				ResourcePolicy newPolicy = resourcePolicyService.clone(context, resourcePolicy);
				newPolicy.setdSpaceObject(dso);
				newPolicy.setAction(resourcePolicy.getAction());
    ...
    private void addCustomPoliciesNotInPlace(Context context, DSpaceObject dso, List<ResourcePolicy> customPolicies)
        ...
        boolean customPoliciesAlreadyInPlace = authorizeService
                .findPoliciesByDSOAndType(context, dso, ResourcePolicy.TYPE_INHERITED).size() > 0;
    ...
    @Override
    public int countVerifiedRows(Context context) throws SQLException {
        return itemDAO.countVerifiedRows(context);
    }

    @Override
    public int countVerifiedAndUpdatedRows(Context context) throws SQLException {
    	return itemDAO.countVerifiedAndUpdatedRows(context);
    }
    
    @Override
	public void checkout(Context context, UUID itemId, EPerson ePerson, String action) throws SQLException, AuthorizeException {
		Item item = find(context, itemId);
		
		if (action.equals("checkout")) {
			if (item.getCheckoutStatus() != null && item.getCheckoutStatus()) {
				throw new IllegalArgumentException("Item already checked out, return item first");
			}
			
			item.setCheckoutStatus(true);
			item.setCheckoutBy(ePerson);
			item.setCheckoutTime(new Date());
			
			// make entry in checkout history
			checkoutHistoryService.checkout(context, item, ePerson, new Date());
		
		} else if (action.equals("checkin")) {
			if (!item.getCheckoutStatus()) {
				throw new IllegalArgumentException("Item already returned, checkout item first");
			}
			
			item.setCheckoutStatus(false);
			item.setCheckoutBy(null);
			item.setCheckoutTime(null);
			
			// make entry in checkout history
			checkoutHistoryService.checkin(context, itemId, ePerson.getID(), new Date());
		} else {
    ...
    @Override
	public List<Item> findByCustomNativeQuery(Context context, String nativeQuery) throws SQLException {
		return itemDAO.findByCustomNativeQuery(context, nativeQuery);
	}

	@Override
	public List<Item> findByCustomNativeQuery(Context context, String nativeQuery, Map<String, Object> params) throws SQLException {
		return itemDAO.findByCustomNativeQuery(context, nativeQuery, params);
	}

	@Override
	public Iterator<Object[]> getItemUploadReport(Context context, List<Community> subCommunities,
			List<Collection> subCollections, MetadataField[] metadataFields) throws SQLException {
		return itemDAO.getItemUploadReport(context, subCommunities, subCollections, metadataFields);
	}

	@Override
	public Iterator<Object[]> getBatchUploadReport(Context context, List<Community> subCommunities,
			List<Collection> subCollections, MetadataField[] metadataFields) throws SQLException {
		return itemDAO.getBatchUploadReport(context, subCommunities, subCollections, metadataFields);
	}

	@Override
	public List<Object[]> findByEachTypeCount(Context context, List<Community> subCommunities,
			List<Collection> subCollections, MetadataField typeField) throws SQLException {
		return itemDAO.findByEachTypeCount(context, subCommunities, subCollections, typeField);
	}
}
