...
public class InstallItemServiceImpl implements InstallItemService {
    ...
    public Item installItem(Context c, InProgressSubmission is,
        ...
        DSpaceObject parent = is.getParent();
        ...
        itemService.inheritCollectionDefaultPolicies(c, item, parent, false);

        Item itemFound = itemService.find(c, item.getID());
        addAuditTrail(c,itemFound, parent);

        return item;
    }
    
    @Override
    public Item installEachItem(Context c, Item item,InProgressSubmission is,
                            String suppliedHandle) throws SQLException,
        AuthorizeException {
    	Date startTime = new Date();
    	String  batchName = item.getBatchName();
        DSpaceObject parent = is.getParent();
        try {
            if (suppliedHandle == null) {
        ...
        log.info(batchName + " *** Start Populate Metadata ***");
        populateMetadata(c, item);
        org.dspace.core.Utils.logTimeDiff(batchName + " *** Populate Metadata Ended ", startTime);
        
        log.info(" *** Finish Item start ***");
        // Finish up / archive the item
        item = finishItem(c, item, is);
        
        org.dspace.core.Utils.logTimeDiff(batchName + " *** Finish Item Ended ", startTime);

        // As this is a BRAND NEW item, as a final step we need to remove the
        // submitter item policies created during deposit and replace them with
        // the default policies from the collection.
        itemService.inheritCollectionDefaultPolicies(c, item, parent, false);
        org.dspace.core.Utils.logTimeDiff(batchName + " *** Replace with default Policies ", startTime);
//        if(isDistrictData) {
//        	districtDataSync(c, item, parent);
//        }
        
        Item itemFound = itemService.find(c, item.getID());
        addAuditTrail(c,itemFound, parent);
//        versionCheck(c,itemFound);
        
//        if(enableSementicSearch) {
//        	rdfGenerationService.updateGraphDB(c, itemFound);
//        }
    ...
    protected Item finishItem(Context c, Item item, InProgressSubmission is)
        throws SQLException, AuthorizeException {
    	// create parent2item mapping
		if (is.getCollection() != null) {			
		...
		} else if (is.getCommunity() != null) {			
			// create community2item mapping
			communityService.addItem(c, is.getCommunity(), item);
			
			// set owning community
	        item.setOwningCommunity(is.getCommunity());
		
		} else {
			throw new RuntimeException("Parent is not present for InProgressSubmission: " + is.getID());
		}
...
//        contentServiceFactory.getInProgressSubmissionService(is).deleteWrapper(c, is);
        ...
        // applying media filter on item
 		try {
 			if (item.getBatchName() != null && !item.getBatchName().contains("single_item")) {
 				mediaFilterService.applyFiltersItem(c, item);
 			}
 		} catch (Exception e) {
 			log.error("Error in Media Filtering", e);
 		}
    ...
    private void addAuditTrail(Context c, Item item, DSpaceObject parent) throws SQLException {  
        auditTrailService.logAction(c, item.getID().toString(), AuditAction.ITEM_CREATED,item.getName(),parent.getName(), "Parent ID : " + parent.getID().toString());
	}
}
