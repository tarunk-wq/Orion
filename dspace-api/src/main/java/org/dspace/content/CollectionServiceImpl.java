...
//            items.remove();
    ...
    @Override
	public void save(Context context, Collection collection) throws SQLException, AuthorizeException {
		collectionDAO.save(context, collection);
	}
    
    @Override
    public void removeCollectionMapping(Context context, Collection collection, Item item) 
	        throws SQLException, AuthorizeException {
	    // Check authorisation
	    authorizeService.authorizeAction(context, collection, Constants.REMOVE);
	    
	    log.info(LogHelper.getHeader(context, "remove_item", "collection_id=" + collection.getID() + ",item_id=" + item.getID()));
