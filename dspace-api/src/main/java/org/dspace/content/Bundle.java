...
public class Bundle extends DSpaceObject implements DSpaceObjectLegacySupport {
    ...
    /**
     * 
     * Bundle Hierarchy (Community-style self-referential mapping)
     * 
     *
     * Intended Idea/Design:
     * - This mapping follows the Community ↔ SubCommunity pattern.
     * - Existing `bundle2bundle` DB table is from an earlier
     *   entity-based design and is NOT compatible.
     * - new join table will be required
     *   before runtime activation.
     */

    /**
     * Child bundles under this bundle.
     */
    @ManyToMany(fetch = FetchType.LAZY)
    @JoinTable(
        name = "bundle2bundle",
        joinColumns = {@JoinColumn(name = "parent_bundle_id")},
        inverseJoinColumns = {@JoinColumn(name = "child_bundle_id")}
    )
    private Set<Bundle> subBundles = new HashSet<>();

    /**
     * Parent bundles of this bundle.
     */
    @ManyToMany(fetch = FetchType.LAZY, mappedBy = "subBundles")
    private Set<Bundle> parentBundles = new HashSet<>();
    ...
    /**
     * Get child bundles under this bundle.
     *
     * @return set of child bundles
     */
    public Set<Bundle> getSubBundles() {
        return subBundles;
    }

    /**
     * Get parent bundles of this bundle.
     *
     * @return set of parent bundles
     */
    public Set<Bundle> getParentBundles() {
        return parentBundles;
    }

    public void removeSubBundle(Bundle subBundle) {
        subBundles.remove(subBundle);
        setModified();
    }

    public void removeParentBundle(Bundle parentBundle) {
        parentBundles.remove(parentBundle);
        setModified();
    }
