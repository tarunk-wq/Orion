...
//        authorizeService.createResourcePolicy(context, newCommunity, anonymousGroup, null, Constants.READ, null);
    ...
    @Override
    public void updateDuplicacyCheck(Context context, UUID subDepartmentId, Boolean enabled)
            throws SQLException, AuthorizeException {

        Community community = find(context, subDepartmentId);

        if (community == null) {
            throw new IllegalArgumentException("Community not found");
        }
        community.setIsDuplicateCheckEnabled(enabled);
    ...
    public void delete(Context context, Community community) throws SQLException, AuthorizeException, IOException {
        ...
        Iterator<Item> items = itemService.findAllByCommunity(context, community);
        while (items.hasNext()) {
            Item item = items.next();
            if (itemService.isOwningCommunity(item, community)) {
                // the community to be deleted is the owning community, thus remove
                // the item from all community it belongs to
                Util.removeItemMappings(context,item);
                itemService.delete(context, item);
            } else {
                // the item was only mapped to this community, so just remove it
                removeItem(context, community, item);
            }
        }
        
        Iterator<WorkspaceItem> workspaceItems = workspaceItemService.findByCommunity(context, community).iterator();
    ...
    protected void rawDelete(Context context, Community community)
        ...
        WorkflowServiceFactory.getInstance().getWorkflowService().deleteCommunity(context, community);
        WorkflowServiceFactory.getInstance().getWorkflowItemService().deleteByCommunity(context, community);
	...
	@Override
	public void save(Context context, Community community) throws SQLException {
		communityDAO.save(context, community);
	}

	@Override
	public void addItem(Context context, Community community, Item item) throws SQLException, AuthorizeException {
		// Check authorisation
        authorizeService.authorizeAction(context, community, Constants.ADD);

        log.info(LogHelper.getHeader(context, "add_item", "community_id="
            + community.getID() + ",item_id=" + item.getID()));

        // Create mapping
        if (!item.getCommunities().contains(community)) {
            item.addCommunity(community);
        }

        context.addEvent(new Event(Event.ADD, Constants.COMMUNITY, community.getID(),
                                   Constants.ITEM, item.getID(), item.getHandle(),
                                   getIdentifiers(context, community)));
	}
	
	@Override
	public void removeItem(Context context, Community community, Item item) throws SQLException, AuthorizeException {
	    // Check authorisation
	    authorizeService.authorizeAction(context, community, Constants.REMOVE);
	    
	    log.info(LogHelper.getHeader(context, "remove_item", "community_id="
	            + community.getID() + ",item_id=" + item.getID()));
	    
	    // Remove mapping
	    if (item.getCommunities().contains(community)) {
	        item.removeCommunity(community);
	    }
	    
	    context.addEvent(new Event(Event.REMOVE, Constants.COMMUNITY, community.getID(), Constants.ITEM, item.getID(), item.getHandle(), getIdentifiers(context, community)));
	}
	
	@Override
    public String getLicense(Community community) {
        String license = getMetadataFirstValue(community, CommunityService.MD_LICENSE, Item.ANY);
	...
	@Override
    public List<Community> findCommunitiesWithSubmit(String q, Context context, Community community,
        ...
        DiscoverResult resp = retrieveCommunitiesWithSubmit(context, discoverQuery, null, community, q);
        for (IndexableObject solrCollections : resp.getIndexableObjects()) {
        	Community c = ((IndexableCommunity) solrCollections).getIndexedObject();
            communities.add(c);
        }
        return communities;
    }
	...
	@Override
    public List<Community> findCommunitiesWithSubmit(String q, Context context, Community community) throws SQLException, SearchServiceException {
    ...
    @Override
    public int countCommunitiesWithSubmit(String q, Context context, Community community)
	...
	private DiscoverResult retrieveCommunitiesWithSubmit(Context context, DiscoverQuery discoverQuery,
