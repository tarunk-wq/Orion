...
public class BundleServiceImpl extends DSpaceObjectServiceImpl<Bundle> implements BundleService {
	...
	public void delete(Context context, Bundle bundle) throws SQLException, AuthorizeException, IOException {
		...
		if (!authorizeService.authorizeActionBoolean(context, getParentBundle(context, bundle), Constants.REMOVE)) {
			authorizeService.authorizeAction(context, bundle, Constants.DELETE);
		}
		ArrayList<String> removedIdentifiers = getIdentifiers(context, bundle);
		String removedHandle = bundle.getHandle();
		UUID removedId = bundle.getID();

		// Check if bundle has hierarchy parent
		Bundle parent = getParentBundle(context, bundle);

		if (parent != null) {
			// First delete sub-bundles recursively
			Iterator<Bundle> subBundles = bundle.getSubBundles().iterator();
			while (subBundles.hasNext()) {
				Bundle subBundle = subBundles.next();
				bundle.removeSubBundle(subBundle);
				delete(context, subBundle);
			}
			// Now let parent remove this bundle
			removeSubBundle(context, parent, bundle);
			return;
		}

		// Register REMOVE event at item level (equivalent of SITE event)
		Item item = (Item) getParentObject(context, bundle);
		if (item != null) {
			context.addEvent(new Event(Event.REMOVE, Constants.ITEM, item.getID(), Constants.BUNDLE, removedId,
					removedHandle, removedIdentifiers));
		}

		// If no hierarchy parent, perform raw delete
		rawDelete(context, bundle);
    ...
    @Override
    public List<Bundle> getSubBundles(Context context, Bundle bundle)
            throws SQLException {
        ...
        return new ArrayList<>(bundle.getSubBundles());
    }
    
	@Override
	public List<Bundle> getAllParentBundles(Context context, Bundle bundle) throws SQLException {

		List<Bundle> parentList = new ArrayList<>();
		Bundle parent = getParentBundle(context, bundle);

		while (parent != null) {
			parentList.add(parent);
			parent = getParentBundle(context, parent);
		}

		return parentList;
	}

	private Bundle getParentBundle(Context context, Bundle bundle) throws SQLException {

		Set<Bundle> parentBundles = bundle.getParentBundles();
		if (CollectionUtils.isNotEmpty(parentBundles)) {
			return parentBundles.iterator().next();
		} else {
			return null;
		}
	}
    ...
    @Override
    public void removeSubBundle(Context context,
                                Bundle parentBundle,
                                Bundle childBundle)
            throws SQLException, AuthorizeException, IOException {

        // Check authorisation
        authorizeService.authorizeAction(context, parentBundle, Constants.REMOVE);

        // Store identifiers before deletion(For logging and event recording) since once we delete the bundle using rawDelete(),the object may not be fully accessible
        ArrayList<String> removedIdentifiers = getIdentifiers(context, childBundle);
        String removedHandle = childBundle.getHandle();
        UUID removedId = childBundle.getID();

        // Physically delete the bundle
        rawDelete(context, childBundle);

        // Log removal
        log.info(LogHelper.getHeader(context, "remove_subbundle",
                "parent_bundle_id=" + parentBundle.getID()
                        + ",child_bundle_id=" + removedId));

        // Register REMOVE event
        context.addEvent(new Event(Event.REMOVE,
                Constants.BUNDLE,
                parentBundle.getID(),
                Constants.BUNDLE,
                removedId,
    ...
    @Override
    public void rawDelete(Context context, Bundle bundle)
            throws SQLException, AuthorizeException, IOException {

        // Log deletion
        log.info(LogHelper.getHeader(context, "delete_bundle",
                "bundle_id=" + bundle.getID()));

        // Register DELETE event
        context.addEvent(new Event(Event.DELETE,Constants.BUNDLE,bundle.getID(),bundle.getHandle(),getIdentifiers(context, bundle)));

        subscribeService.deleteByDspaceObject(context, bundle);

        // delete bitstreams (mirror remove collections pattern from community)
        ...
        // Remove any Handle
        handleService.unbindHandle(context, bundle);
        
        // Remove parent-child relationships (hierarchy cleanup)
        Bundle parent = (Bundle) getParentBundle(context, bundle);
        if (parent != null) {
            bundle.removeParentBundle(parent);
            parent.removeSubBundle(bundle);
        }

        List<Item> items = new LinkedList<>(bundle.getItems());
        bundle.getItems().clear();
        for (Item owningItem : items) {
            owningItem.removeBundle(bundle);
        }
    ...
    @Override
    public Bundle createSubBundle(Context context, Bundle parentBundle)
            throws SQLException, AuthorizeException {

        return createSubBundle(context, parentBundle, null);
    }

    @Override
    public Bundle createSubBundle(Context context, Bundle parentBundle, String name)
            throws SQLException, AuthorizeException {

        return createSubBundle(context, parentBundle, name, null);
    }
    ...
    public Bundle createSubBundle(Context context, Bundle parentBundle,
        ...
        // Check authorisation
        authorizeService.authorizeAction(context, parentBundle, Constants.ADD);

        Bundle b;

        Item owningItem = (Item) getParentObject(context, parentBundle);

        b = create(context, owningItem, name);

        addSubBundle(context, parentBundle, b);

        return b;
    }

    @Override
    public void addSubBundle(Context context, Bundle parentBundle,
                             Bundle childBundle)
            throws SQLException, AuthorizeException {
        ...
        log.info(LogHelper.getHeader(context, "add_subbundle",
                "parent_bundle_id=" + parentBundle.getID()
                        + ",child_bundle_id=" + childBundle.getID()));

        if (!parentBundle.getSubBundles().contains(childBundle)) {
            parentBundle.getSubBundles().add(childBundle);
            childBundle.getParentBundles().add(parentBundle);
        }

        context.addEvent(new Event(Event.ADD, Constants.BUNDLE,
                parentBundle.getID(), Constants.BUNDLE,
                childBundle.getID(),
                parentBundle.getHandle(),
                getIdentifiers(context, parentBundle)));
    }
