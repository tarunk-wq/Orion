...
public class Role implements BeanNameAware {
    ...
    public RoleMembers getMembers(Context context, XmlWorkflowItem wfi) throws SQLException {
        if (scope == Scope.REPOSITORY) {
        ...
        } else if (scope == Scope.COLLECTION || scope == Scope.COMMUNITY) {
        	if (wfi.getCollection() != null) {
        		...
        		CommunityRole communityRole = communityRoleService.find(context, wfi.getCommunity().getID(), id);
        		if (communityRole != null) {
        			RoleMembers assignees = new RoleMembers();
        			assignees.addGroup(communityRole.getGroup());
        			return assignees;
        		}
