...
public class WorkflowUtils extends Util {
    ...
    protected static final CommunityRoleService communityRoleService =
            XmlWorkflowServiceFactory.getInstance().getCommunityRoleService();
    ...
    /**
     * Creates a role for a community by linking a group of epersons to a role ID
     *
     * @param context    The relevant DSpace Context.
     ...
     * @throws SQLException       An exception that provides information on a database access error or other errors.
     */
    public static void createCommunityWorkflowRole(Context context, Community community, String roleId, Group group)
        throws AuthorizeException, SQLException {
    	CommunityRole communityRole = communityRoleService.create(context, community, roleId, group);
    	communityRoleService.update(context, communityRole);
    }
	...
	public static Map<String, Role> getCommunityAndRepositoryRoles(Community thisCommunity)
			throws IOException, WorkflowConfigurationException, SQLException {
		Workflow workflow = xmlWorkflowFactory.getWorkflow(thisCommunity);
		LinkedHashMap<String, Role> result = new LinkedHashMap<>();
		if (workflow != null) {
			// Make sure we find one
			Map<String, Role> allRoles = workflow.getRoles();
			// We have retrieved all our roles, not get the ones which can be configured by
			// the collection
			for (String roleId : allRoles.keySet()) {
				Role role = allRoles.get(roleId);
				// We just require the roles which have a scope of collection
				if ((role.getScope() == Role.Scope.COMMUNITY || role.getScope() == Role.Scope.REPOSITORY)
						&& !role.isInternal()) {
					// for community actions defined will be comm-action, extract comm- from action
					if (roleId.contains("comm-")) {						
    ...
    public static Group getRoleGroup(Context context, DSpaceObject dSpaceObject, Role role) throws SQLException {
        if (role.getScope() == Role.Scope.REPOSITORY) {
            ...
            CollectionRole collectionRole = collectionRoleService.find(context, (Collection) dSpaceObject, role.getId());
            if (collectionRole == null) {
                return null;
            }

            return collectionRole.getGroup();
        }  else if (role.getScope() == Role.Scope.COMMUNITY) {
        	String roleId = role.getId();
        	// for community actions defined will be comm-action, extract comm- from action
			if (roleId.contains("comm-")) {						
				roleId = roleId.substring(5);
			}
            CommunityRole communityRole = communityRoleService.find(context, ((Community) dSpaceObject).getID(), roleId);
            if (communityRole == null) {
                return null;
            }

            return communityRole.getGroup();
