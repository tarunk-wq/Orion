...
//        }
    ...
    public <M, R> R toRest(Context context, M modelObject, Projection projection, boolean isSingleItem) {
    	
    	List<ItemRest> listOfItemRest = new ArrayList<ItemRest>();
    	if(isSingleItem) {
        	Item item = (Item) modelObject;
        	DiscoveryMoreLikeThisConfiguration discoveryMoreLikeThisConfiguration = new DiscoveryMoreLikeThisConfiguration();
        	discoveryMoreLikeThisConfiguration.setMax(10);
			
			discoveryMoreLikeThisConfiguration.setMinTermFrequency(1);
			discoveryMoreLikeThisConfiguration.setMinWordLength(1);
			List<String> similarityMetadataFields = new ArrayList<String>();
			similarityMetadataFields.add("dc.title");
			similarityMetadataFields.add("dc.contributor.author");
			similarityMetadataFields.add("dc.casetype");
			discoveryMoreLikeThisConfiguration.setSimilarityMetadataFields(similarityMetadataFields);
			List<Item> moreLikeItems = searchService.getRelatedItems(context, item, discoveryMoreLikeThisConfiguration);
			if (moreLikeItems.size() > 0 && moreLikeItems.size() == 10) {
				for (Item moreLikeItem : moreLikeItems) {
					Projection projectionMoreLike = utils.obtainProjection();
					M transformedModelMoreLike = projectionMoreLike.transformModel((M)moreLikeItem);
			        DSpaceConverter<M, R> converterMoreLike = requireConverter(moreLikeItem.getClass());
			        R restObjectMoreLike = converterMoreLike.convert(transformedModelMoreLike, projectionMoreLike);
			        listOfItemRest.add((ItemRest) restObjectMoreLike);
				}
			}
        }
        M transformedModel = projection.transformModel(modelObject);
        ...
        itemRest.setListOfItemsRest(listOfItemRest);
        
        restObject = (R) itemRest;
        if (restObject instanceof BaseObjectRest) {
            BaseObjectRest baseObjectRest = (BaseObjectRest) restObject;
            // This section will verify whether the current user has permissions to retrieve the
            // rest object. It'll only return the REST object if the permission is granted.
