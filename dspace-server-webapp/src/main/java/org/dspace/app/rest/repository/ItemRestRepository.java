...
public class ItemRestRepository extends DSpaceObjectRestRepository<Item, ItemRest> {
    ...
    @Override
    @PreAuthorize("hasPermission(#id, 'ITEM', 'STATUS') || hasPermission(#id, 'ITEM', 'READ') || hasAuthority('AUTHENTICATED')")
    public ItemRest findOne(Context context, UUID id) {
        ...
        try {
            ...
            if (item == null) {
                throw new DSpaceBadRequestException("Item not found: " + id);
            }
            ...
            itemRest = converter.toRest(context, item, utils.obtainProjection(), true);
            
            EPerson currentUser = context.getCurrentUser();
            if(currentUser != null) {
            	determineItemViewPermission(context, item, currentUser.getEmail(), itemRest);
            }
            
            // adding entry in audittrail
            Utils.addAuditTrail(context, item, item.getOwningParent(), AuditAction.ITEM_RETRIEVED,item.getName(),item.getOwningParent().getName());
            context.commit();
            return itemRest;
    ...
    protected void delete(Context context, UUID id) throws AuthorizeException {
        ...
        try {
        	 // adding entry in audittrail
        	Utils.addAuditTrail(context, item, item.getOwningParent(), AuditAction.ITEM_DELETE,item.getName(),item.getOwningParent().getName());
            deleteMultipleRelationshipsCopyVirtualMetadata(context, copyVirtual, item);
            Utils.removeItemMappings(context,item);
    ...
    private void determineItemViewPermission(Context context, Item item, String userEmail, ItemRest itemRest) throws SQLException {
        try {
            if (userpermissionService.hasAdminPermission(context, item)) {
                itemRest.setHasCheckInPrivilege(true);
                return;
            }
        } catch (Exception e) {
            log.error("Error checking admin permissions for item: " + item.getID(), e);
            return;
        }
        
        TempAccess tempAccess = tempAccessService.findByItemAndUser(context, item.getID(), userEmail);
        if (tempAccess == null) {
            log.info("No TempAccess entry found for user: " + userEmail);
            return;
        }
        
        LocalDate today = LocalDate.now();
        LocalDate startDate = tempAccess.getStartDate();
        LocalDate endDate = tempAccess.getEndDate();
        
        // Check if TempAccess has expired
        if (endDate != null && today.isAfter(endDate)) {
            try {
                tempAccessService.deleteByItemAndUser(context, item.getID(), userEmail);
                context.commit();
                log.info("Expired TempAccess removed for user: " + userEmail);
            } catch (Exception deleteEx) {
                context.abort();
                log.error("Error deleting expired TempAccess for user: " + userEmail, deleteEx);
            }
            return;
        }
        
        // Check if TempAccess has not started yet
        if (startDate != null && startDate.isAfter(today)) {
            log.warn("Access to view Item not yet started for user: " + userEmail);
            throw new ResponseStatusException(HttpStatus.UNAUTHORIZED, "Access not started yet");
        }
    }
}
