...
public class VocabularyEntryLinkRepository extends AbstractDSpaceRestRepository
    ...
    private Choice getFirstChoiceOrDefault(Choices choices, String filter) {
	    if (choices != null && choices.values != null && choices.values.length > 0) {
	        return choices.values[0];
	    }
	    return new Choice(filter, filter, filter);
	}
}
