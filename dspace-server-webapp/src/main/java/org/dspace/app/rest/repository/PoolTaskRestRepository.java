...
@Component(PoolTaskRest.CATEGORY + "." + PoolTaskRest.PLURAL_NAME)
public class PoolTaskRestRepository extends DSpaceRestRepository<PoolTaskRest, Integer>
	...
	{
		try {	
	        int total = poolTaskService.countRows(context);
			List<PoolTask> pooltasklist=poolTaskService.findAll(context,pageable.getPageSize(),
                    Math.toIntExact(pageable.getOffset()));
			return converter.toRestPage(pooltasklist,pageable,total, utils.obtainProjection());
		} catch (Exception e) {
			throw new RuntimeException(e.getMessage(), e);
		}
	}
