...
public class CommunityRestRepository extends DSpaceObjectRestRepository<Community, CommunityRest> {
	...
	public GroupRest createWorkflowGroupForRole(Context context, HttpServletRequest request, Community community,
			String workflowRole) throws AuthorizeException, SQLException, IOException, WorkflowConfigurationException {
        context.turnOffAuthorisationSystem();
        Group group = workflowService.createWorkflowRoleGroup(context, community, workflowRole);
	...
	private GroupRest populateGroupInformation(Context context, HttpServletRequest request, Group group)
		...
		try {
			ServletInputStream input = request.getInputStream();
			groupRest = ObjectMapperUtil.readValue(input, GroupRest.class);
			if (groupRest.isPermanent() || StringUtils.isNotBlank(groupRest.getName())) {
				throw new UnprocessableEntityException(
						"The given GroupRest object has to be non-permanent and can't" + " contain a name");
			}
			MetadataRest metadata = groupRest.getMetadata();
			SortedMap<String, List<MetadataValueRest>> map = metadata.getMap();
			if (map != null) {
	...
	public GroupRest getWorkflowGroupForRole(Context context, Community community, String workflowRole)
			throws SQLException, IOException, WorkflowConfigurationException, AuthorizeException, WorkflowException {

		if (WorkflowUtils.getCommunityAndRepositoryRoles(community).get(workflowRole) == null) {
			throw new ResourceNotFoundException(
					"Couldn't find role for: " + workflowRole + " in the community with UUID: " + community.getID());
		}
	...
	public void deleteWorkflowGroupForRole(Context context, HttpServletRequest request, Community community,
			String workflowRole)
		...
		List<CommunityRole> communityRoles = communityRoleService.findByGroup(context, group.getID());
		if (!communityRoles.isEmpty()) {
			communityRoles.stream().forEach(communityRole -> {
				try {
					communityRoleService.delete(context, communityRole);
				} catch (SQLException e) {
