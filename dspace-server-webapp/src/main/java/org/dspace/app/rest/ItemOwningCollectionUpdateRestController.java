...
public class ItemOwningCollectionUpdateRestController {
    ...
    public DSpaceObjectRest move(@PathVariable UUID uuid,
        ...
        if (dsoList.size() != 1) {
            throw new UnprocessableEntityException("Exactly one target Collection or Community must be provided.");
        }
        
        DSpaceObject target = dsoList.get(0);
        if (target.getType() != Constants.COLLECTION && target.getType() != Constants.COMMUNITY) {
            throw new UnprocessableEntityException("Target must be a Collection or Community.");
        }
        ...
        if (!(item.isArchived() || item.isWithdrawn())) {
            throw new DSpaceBadRequestException("Only archived or withdrawn items can be moved");
        }
        
        DSpaceObject result;
        if (target instanceof Collection) {
        	Collection collection = (Collection) target;
            if (item.getOwningCollection() != null && collection.equals(item.getOwningCollection())) {
                throw new DSpaceBadRequestException("Already in this collection");
            }
            if (item.getOwningCollection() != null) {
                result = moveItemToCollection(context, item, collection); // Collection to Collection
            } else {
                Community fromCommunity = item.getOwningCommunity();
                result = moveItemCommunityToCollection(context, item, fromCommunity, collection); // Community to Collection
            }
        } else {
        	Community community = (Community) target;
            if (item.getOwningCommunity() != null && community.equals(item.getOwningCommunity())) {
                throw new DSpaceBadRequestException("Already in this community");
            }
            if (item.getOwningCommunity() != null) {
                result = moveItemToCommunity(context, item, community); // Community to Community
            } else {
                Collection fromCollection = item.getOwningCollection();
                result = moveItemCollectionToCommunity(context, item, fromCollection, community); // Collection to Community
            }
        }
        
        audittrailService.logAction(context, uuid.toString(), AuditAction.ITEM_MOVED, item.getName(), target.getName());
        return converter.toRest(result, utils.obtainProjection());

    }
    
    private Collection moveItemToCollection(final Context context, final Item item, final Collection targetCollection) throws SQLException, IOException, AuthorizeException {
    ...
    private Community moveItemToCommunity(final Context context, final Item item, final Community targetCommunity) throws SQLException, IOException, AuthorizeException {
		Community currentCommunity = item.getOwningCommunity();
		if (authorizeService.authorizeActionBoolean(context, currentCommunity, Constants.ADMIN)) {
			itemService.moveToCommunity(context, item, currentCommunity, targetCommunity);
			context.commit();
    ...
    private Community moveItemCollectionToCommunity(Context context, Item item, Collection fromCollection, Community targetCommunity) throws SQLException, IOException, AuthorizeException {
    	if (authorizeService.authorizeActionBoolean(context, fromCollection, Constants.ADMIN)) {
    		itemService.moveItemCollectionToCommunity(context, item, fromCollection, targetCommunity);
			context.commit();
			return context.reloadEntity(targetCommunity);
    	}
    	return null;
    ...
    private Collection moveItemCommunityToCollection(Context context, Item item, Community fromCommunity, Collection targetCollection) throws SQLException, IOException, AuthorizeException {
    	if (authorizeService.authorizeActionBoolean(context, fromCommunity, Constants.ADMIN)) {
    		itemService.moveItemCommunityToCollection(context, item, fromCommunity, targetCollection);
			context.commit();
			return context.reloadEntity(targetCollection);
    	}
    	return null;
