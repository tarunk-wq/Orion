    ...
    public RegistrationRest createAndReturn(Context context) {
        ...
        if (eperson != null && accountType.equalsIgnoreCase(TYPE_FORGOT)) {
            try {
            	...
            	if(!eperson.canLogIn() && unsuccessfulAttempts >= unsuccessfulAttemptCount) {
                    throw new DisabledException("Account locked for " + eperson.getEmail() 
									+ " due to multiple failed login attempts. Try again after lockout period ends.");
            	}
            	
                if (!AuthorizeUtil.authorizeUpdatePassword(context, eperson.getEmail())) {
                	throw new AuthorizeException("EPerson is disabled from login with email: " + eperson.getEmail());
                }
        ...
        } else if (eperson == null && accountType.equalsIgnoreCase(TYPE_FORGOT)) {
        	throw new DSpaceBadRequestException("User with given email does not exist..");        	
        }
...
//                            + registrationRest.getEmail(), e);
//                }
            	throw new DSpaceBadRequestException("User with given email already exists..");
            }
        }
        return null;
    }
