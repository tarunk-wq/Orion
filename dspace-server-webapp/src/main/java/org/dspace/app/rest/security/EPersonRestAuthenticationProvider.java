    ...
    private Authentication authenticateNewLogin(Context context, Authentication authentication) {
        ...
        if (authentication != null) {
            ...
            if (implicitStatus == AuthenticationMethod.SUCCESS) {
                ...
                }  else if (AuthenticationMethod.LOCK_OUT == authenticateResult) {
                	log.warn(LogHelper.getHeader(context, "failed_login", "email="
                            + name + ", result="
                            + authenticateResult));
                	throw new LockedException("User Locked out due to Unsuccessful attempts!");
                } else if (AuthenticationMethod.PASSWORD_EXPIRED == authenticateResult) {
                	log.warn(LogHelper.getHeader(context, "failed_login", "email="
                            + name + ", result="
                            + authenticateResult));
                	throw new CredentialsExpiredException("User Password Expired!");
                } else if (AuthenticationMethod.USER_DISABLED == authenticateResult) {
                	log.warn(LogHelper.getHeader(context, "failed_login", "email="
                            + name + ", result="
                            + authenticateResult));
                	throw new DisabledException("User Disabled from Login!");
                }
                else {
