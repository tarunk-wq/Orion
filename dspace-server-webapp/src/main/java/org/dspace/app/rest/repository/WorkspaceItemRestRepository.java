...
public class WorkspaceItemRestRepository extends DSpaceRestRepository<WorkspaceItemRest, Integer>
    ...
    public WorkspaceItemRest upload(HttpServletRequest request, String apiCategory, String model, Integer id,
    	...
    	String[] bundleParams = request.getParameterValues("bundles");
    	List<String> bundleNames = null;

    	if (bundleParams != null) {
    	    bundleNames = Arrays.asList(bundleParams);
    	}
        ...
        Utils.addAuditTrail(context,source.getItem(), source.getCollection(), AuditAction.BITSTREAM_UPLOAD_ITEM_SUBMISSION,item.getID().toString(), source.getParent().getName());
        context.commit();
        return wsi;
    }
    ...
    public void patch(Context context, HttpServletRequest request, String apiCategory, String model, Integer id,
        ...
        //Check for duplicate items by its unique metadata
        PatchMetadataBundle metadataBundle = submissionService.collectMetadataFromPatch(operations);  
        Map<String, List<String>> metadataMap = metadataBundle.getMetadataMap();
        if(!metadataMap.isEmpty()) {
        	boolean checkIfDuplicateItemsExist = submissionService.checkForDuplicateItems(context, id, metadataMap);
            if(checkIfDuplicateItemsExist) {
            	throw new DuplicateEntityException("A duplicate item already exist. Change metadata to deposit item.");
            }
        }
    ...
    public Iterable<WorkspaceItemRest> upload(Context context, HttpServletRequest request,
        ...
        if (!result.isEmpty()) {
            for (WorkspaceItem wi : result) {
                            ...
                            if (UploadableStep.class.isAssignableFrom(stepClass)) {
                                ...
                                for (MultipartFile mpFile : uploadfiles) {
                                    ErrorRest err = uploadableStep.upload(context,
                                        submissionService, stepConfig, wi, mpFile,bundleNames);
                                    if (err != null) {
                                        errors.add(err);
                                    }
                                }
                ...
                Utils.addAuditTrail(context,source.getItem(), source.getCollection(), AuditAction.BITSTREAM_UPLOAD_ITEM_SUBMISSION,source.getItem().getID().toString(), source.getParent().getName());
                results.add(wsi);
            }
        }
        return results;
