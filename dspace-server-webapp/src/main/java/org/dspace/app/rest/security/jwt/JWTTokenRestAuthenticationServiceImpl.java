...
public class JWTTokenRestAuthenticationServiceImpl implements RestAuthenticationService, InitializingBean {
    ...
    public void addAuthenticationDataForUser(HttpServletRequest request, HttpServletResponse response,
        ...
        try {
            ...
            EPerson eperson = ePersonService.findByEmail(context, authentication.getName());
            context.setCurrentUser(eperson);

            String token = loginJWTTokenHandler.createTokenForEPerson(context, request,
                                                                 authentication.getPreviousLoginDate(), false);
            
            // adding entry in audittrail
            audittrailService.logAction(context, "", AuditAction.LOGIN);
            
            if(eperson.getPasswordExpiryDate() == null) {
    	        ...
    	        try {
					...
					log.error(LogHelper.getHeader(context, "authenticate", "Error updating password expiry date for user" + 
            ...
            int remindDay = DSpaceServicesFactory.getInstance().getConfigurationService().getIntProperty("eperson.password.reminder.day", 0);
            
            Date expiryDate = eperson.getPasswordExpiryDate();
            ...
            if (expiryDate != null) {
                ...
                if (diffDays < 0) {
                    status = 2; // expired
                } else if (diffDays <= remindDay) {
                    status = 1; // about to expire
                }
            }
            
            userObj.put("status", status);
            userObj.put("expiryDate", eperson.getPasswordExpiryDate().toString());
    ...
    public AuthenticationToken getShortLivedAuthenticationToken(Context context, HttpServletRequest request) {
        try {
            String token = shortLivedJWTTokenHandler.createTokenForEPerson(context, request, null, true);
            context.commit();
    ...
    public EPerson getAuthenticatedEPerson(HttpServletRequest request, HttpServletResponse response, Context context) {
        try {
            ...
            if (token == null) {
                ...
                ePerson = shortLivedJWTTokenHandler.parseEPersonFromToken(token, request, context, true);
            } else {
                ePerson = loginJWTTokenHandler.parseEPersonFromToken(token, request, context, false);
            }
            return ePerson;
        } catch (JOSEException e) {
    ...
    public void invalidateAuthenticationData(HttpServletRequest request, HttpServletResponse response,
        ...
        // adding entry in audittrail
        audittrailService.logAction(context, "", AuditAction.LOGOUT);
