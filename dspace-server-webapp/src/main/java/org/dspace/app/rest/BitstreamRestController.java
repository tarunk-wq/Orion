...
public class BitstreamRestController {
    ...
    protected VersioningBitstreamService versioningBitstreamService;
    ...
    public ResponseEntity retrieve(@PathVariable UUID uuid,
    	...
    	//Checking if any version of this BITSTREAM is available.
        Bitstream oldBitstream = bitstreamService.find(context, uuid);
        
//        if(Utils.checkIfAnnotatedBitstreamExist(context,currentUser,oldBitstream) && action == null &&  downloadType == null) {
        if(Utils.checkIfAnnotatedBitstreamExist(context,currentUser,oldBitstream) && (action == null || (downloadType != null && downloadType.equalsIgnoreCase("version")))) {
        	UserBitstream userBitstream = userBitstreamService.findByEperson(context, currentUser, oldBitstream).get(0);
        	uuid = userBitstream.getVersionedBistream().getID();
        }
        ...
        try {
            ...
            // download/streaming
          //Log the download or view bitstream action
            audittrailService.logAction(context, String.valueOf(bit.getID()), AuditAction.BITSTREAM_DOWNLOAD_VIEWED, bit.getName());
    ...
    public BitstreamResource updateBitstreamFormat(@PathVariable UUID uuid,
     ...
     // adding entry in audittrail
        audittrailService.logAction(context, String.valueOf(bitstream.getID()), AuditAction.BITSTREAM_UPDATE_FORMAT, bitstream.getName());
    ...
    @GetMapping("/versions")
    public ResponseEntity<ArrayNode> retrieveVersions(@PathVariable UUID uuid,
                         HttpServletRequest request) throws Exception {
		
		Context context = ContextUtil.obtainContext(request);
		List<VersionBitstream> list = versioningBitstreamService.findAllVersions(context, uuid);
    	return new ResponseEntity<>(convertVersions(context, list), HttpStatus.OK);
    }
    
    private ArrayNode convertVersions(Context context, List<VersionBitstream> list) throws SQLException {
    	ArrayNode nodes = mapper.createArrayNode();
    	
    	for (VersionBitstream versionBitstream : list) {
    		ObjectNode node = mapper.createObjectNode();
    		
    		node.put("bitstreamId", versionBitstream.getBitstream().getID().toString());
    		node.put("name", versionBitstream.getBitstreamName());
    		node.put("version", versionBitstream.getVersionNumber());
    		node.put("creator", versionBitstream.getePerson().getName());
    		node.put("date", Utils.convertDateToString(versionBitstream.getVersionDate()));
    		node.put("active", versionBitstream.getActiveVersion());
    		node.put("comment", versionBitstream.getComment());
    		node.put("mimeType", versionBitstream.getBitstream().getFormat(context).getMIMEType());
