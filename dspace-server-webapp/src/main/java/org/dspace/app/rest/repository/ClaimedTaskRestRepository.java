...
public class ClaimedTaskRestRepository extends DSpaceRestRepository<ClaimedTaskRest, Integer>
    ...
    protected ClaimedTaskRest action(Context context, HttpServletRequest request, Integer id)
        ...
        String batchName="";
        try {
			 wi = xmlWorkflowItemService.find(context, id);
			 batchName = wi.getBatchName();
			 
		} catch (SQLException e) {
			log.error("Error in fetching XmlWorkflowItem for ID ::" + id, e);
		}
        if (wi != null) {
        	log.info("WorkflowItem Found for ID :: " + id);
        	List<ClaimedTask> tasks = claimedTaskService.findByWorkflowItem(context, wi);
        	if (tasks != null && !tasks.isEmpty()) {
        		log.info("Claimed Task found for WorkflowItem :: " + id);
        		task = tasks.get(0);
        	}
        } else {
        	log.info("WorkflowItem not found :: " + id);
        	log.info("Trying to search in claimed Task Table...");
        	task = claimedTaskService.find(context, id);
        }
        if (task == null) {
        ...
        Workflow workflow;
        try {
            workflow = factory.getWorkflowFactory().getWorkflow(task.getWorkflowItem().getParent());
            ...
            log.info(batchName +  " *** Do state Service Started");
            workflowService
    ...
    protected void delete(Context context, Integer id) {
        ...
        try {
			...
			log.error("Error in finding workflow item",e);
		}    
        try {
        	if (wi != null) {
             	List<ClaimedTask> tasks = claimedTaskService.findByWorkflowItem(context, wi);
    ...
    protected ClaimedTaskRest createAndReturn(Context context, List<String> list)
        ...
        try {
            ...
            Workflow workflow = factory.getWorkflowFactory().getWorkflow(task.getWorkflowItem().getParent());
            Step step = workflow.getStep(task.getStepID());
            WorkflowActionConfig currentActionConfig = step.getActionConfig(task.getActionID());
