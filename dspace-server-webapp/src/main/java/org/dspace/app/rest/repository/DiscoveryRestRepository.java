...
public class DiscoveryRestRepository extends AbstractDSpaceRestRepository {
    ...
    public SearchResultsRest getSearchObjects(final String query, final List<String> dsoTypes, final String dsoScope,
        ...
        try {
        	if(searchFilters.size() > 0 && searchFilters.get(0).getOperator().equalsIgnoreCase("soundslike")) {
            	String phoneticQuery="";
            		int index=0;
            		for(SearchFilter filter: searchFilters) {
            			if(index<searchFilters.size()-1) {
            				String q = filter.getName()+"_ptc:"+filter.getValue()+" AND ";
                    		phoneticQuery+=q;
            			}else {
            				String q = filter.getName()+"_ptc:"+filter.getValue();
                    		phoneticQuery+=q;
            			}
                		index++;
                	}
            	discoverQuery = queryBuilder
                        .buildQuery(context, scopeObject, discoveryConfiguration, phoneticQuery, searchFilters, dsoTypes, page);
            }else if(searchFilters.size() > 0 && searchFilters.get(0).getOperator().equalsIgnoreCase("fuzzylike")) {
            	String fuzzyQuery="";
        		int index=0;
        		for(SearchFilter filter: searchFilters) {
        			if(index<searchFilters.size()-1) {
        				String q = filter.getName()+"_fuzzy_searchable:"+filter.getValue()+"~2 AND ";
                		fuzzyQuery+=q;
        			}else {
        				String q = filter.getName()+"_fuzzy_searchable:"+filter.getValue()+"~2";
                		fuzzyQuery+=q;
        			}
            		index++;
            	}
        		discoverQuery = queryBuilder
                    .buildQuery(context, scopeObject, discoveryConfiguration, fuzzyQuery, searchFilters, dsoTypes, page);
            }else {
            	discoverQuery = queryBuilder
