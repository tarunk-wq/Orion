...
public class ItemConverter
    ...
    public ItemRest convert(Item obj, Projection projection) {
        ...
        item.setBatchName(obj.getBatchName());
        //Get Owning Collection and Community UUIDs
        try {
            Collection collection = obj.getOwningCollection();
            if (collection != null) {
                item.setOwningCollectionId(collection.getID().toString());
                item.setOwningCollectionName(collection.getName());
            }
            if(obj.getOwningCommunity() != null) {            	
            	item.setOwningCommunityName(obj.getOwningCommunity().getName());
            	item.setOwningCommunityId(obj.getOwningCommunity().getID().toString());
            }
        } catch (Exception e) {
            log.error("Error setting owning collection/community UUIDs for item {}", obj.getID(), e);
        }
        ...
        item.setCheckoutStatus(obj.getCheckoutStatus() == null ? false : obj.getCheckoutStatus());
        if (obj.getCheckoutBy() != null) {       	
        	item.setCheckoutBy(obj.getCheckoutBy().getID());
        	item.setCheckoutByUserName(obj.getCheckoutBy().getEmail());
        }
        item.setCheckoutTime(obj.getCheckoutTime());
