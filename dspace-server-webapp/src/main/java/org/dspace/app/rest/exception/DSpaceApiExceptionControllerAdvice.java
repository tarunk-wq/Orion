...
public class DSpaceApiExceptionControllerAdvice extends ResponseEntityExceptionHandler {
    ...
    protected void handleAuthorizeException(HttpServletRequest request, HttpServletResponse response, Exception ex)
        ...
        if (Objects.nonNull(context.getCurrentUser())) {
            sendErrorResponse(request, response, ex, ex.getMessage() != null && !ex.getMessage().isEmpty() ? ex.getMessage() : "Access is denied", HttpServletResponse.SC_FORBIDDEN);
        } else {
            sendErrorResponse(request, response, ex, ex.getMessage() != null && !ex.getMessage().isEmpty() ? ex.getMessage() : "Authentication is required", HttpServletResponse.SC_UNAUTHORIZED);
        }
    }
    ...
    protected void csrfTokenException(HttpServletRequest request, HttpServletResponse response, Exception ex)
        throws IOException {
        sendErrorResponse(request, response, ex, ex.getMessage() != null && !ex.getMessage().isEmpty() ? ex.getMessage() : "Access is denied. Invalid CSRF token.",
    ...
    protected void handleWrongRequestException(HttpServletRequest request, HttpServletResponse response,
        ...
        sendErrorResponse(request, response, ex, ex.getMessage() != null && !ex.getMessage().isEmpty() ? ex.getMessage() : "Request is invalid or incorrect", HttpServletResponse.SC_BAD_REQUEST);
    }
    ...
    protected void handleSQLException(HttpServletRequest request, HttpServletResponse response, Exception ex)
        ...
        sendErrorResponse(request, response, ex,
        		ex.getMessage() != null && !ex.getMessage().isEmpty() ? ex.getMessage() : "An internal database error occurred", HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
    }
    ...
    protected void handleIOException(HttpServletRequest request, HttpServletResponse response, Exception ex)
        throws IOException {
        sendErrorResponse(request, response, ex,
        		ex.getMessage() != null && !ex.getMessage().isEmpty() ? ex.getMessage() : "An internal read or write operation failed",
    ...
    protected void methodNotAllowedException(HttpServletRequest request, HttpServletResponse response,
                                                  Exception ex) throws IOException {
        sendErrorResponse(request, response, ex, ex.getMessage() != null && !ex.getMessage().isEmpty() ? ex.getMessage() : "Method is not allowed or supported",
                          HttpServletResponse.SC_METHOD_NOT_ALLOWED);
    }
    
    @ExceptionHandler(DuplicateEntityException.class)
    protected void handleDuplicateSection(HttpServletRequest request, HttpServletResponse response,
                                                  Exception ex) throws IOException {
        sendErrorResponse(request, response, ex, ex.getMessage(),
    ...
    protected void handleUnprocessableEntityException(HttpServletRequest request, HttpServletResponse response,
        ...
        sendErrorResponse(request, response, ex,
        		ex.getMessage() != null && !ex.getMessage().isEmpty() ? ex.getMessage() : "Unprocessable or invalid entity",
    ...
    protected void handleInvalidSearchRequestException(HttpServletRequest request, HttpServletResponse response,
                                                      Exception ex) throws IOException {
        sendErrorResponse(request, response, ex,
        		ex.getMessage() != null && !ex.getMessage().isEmpty() ? ex.getMessage() : "Invalid search request",
                HttpStatus.UNPROCESSABLE_ENTITY.value());
    }
    ...
    protected void ParameterConversionException(HttpServletRequest request, HttpServletResponse response, Exception ex)
        ...
        sendErrorResponse(request, response, ex,
        		ex.getMessage() != null && !ex.getMessage().isEmpty() ? ex.getMessage() : "A required parameter is invalid",
    ...
    protected void MissingParameterException(HttpServletRequest request, HttpServletResponse response, Exception ex)
        ...
        // we want the 400 status for missing parameters, see https://github.com/DSpace/DSpace/issues/7765
        sendErrorResponse(request, response, ex,
        		ex.getMessage() != null && !ex.getMessage().isEmpty() ? ex.getMessage() : "A required parameter is missing",
                          HttpStatus.BAD_REQUEST.value());
    }
    ...
    protected void handleInvalidCaptchaTokenRequestException(HttpServletRequest request, HttpServletResponse response,
                                                                                      Exception ex) throws IOException {
        sendErrorResponse(request, response, ex, ex.getMessage() != null && !ex.getMessage().isEmpty() ? ex.getMessage() : "Invalid captcha token", SC_FORBIDDEN);
    }
    ...
    @ExceptionHandler({DSpaceBadRequestException.class})
    protected void handleIDSpaceBadRequestException(HttpServletRequest request, HttpServletResponse response,
                                               Exception ex) throws IOException {
    ...
    protected void handleGenericException(HttpServletRequest request, HttpServletResponse response, Exception ex)
        ...
        sendErrorResponse(request, response, ex, ex.getMessage() != null && !ex.getMessage().isEmpty() ? ex.getMessage() : "An exception has occurred", returnCode);

    }
    
    @ExceptionHandler(HttpStatusException.class)
    protected void handleHttpStatusException(HttpServletRequest request, HttpServletResponse response, HttpStatusException ex)
        throws IOException {
    ...
    protected void handleInvalidCaptchaTokenRequestException(HttpServletRequest request, HttpServletResponse response,
        ...
        sendErrorResponse(request, response, ex, ex.getMessage(), ex.getStatusCode().value());
    }
