...
public class BundleRestRepository extends DSpaceObjectRestRepository<Bundle, BundleRest> {
    ...
    @Autowired
    private VersioningBitstreamService vbService;
    ...
    public BitstreamRest uploadBitstream(Context context, Bundle bundle, String fileName, InputStream fileInputStream,
        ...
        try {
        	...
        	String message = "Something went wrong with trying to create the media filter of single bitstream for file with filename: "
                    + fileName
                    + " for item with uuid: " + bundle.getID() + " and possible properties: " + properties;
            log.error(message, e);
            throw new RuntimeException(message, e);		}
    ...
    private Bitstream processBitstreamCreation(Context context, Bundle bundle, InputStream fileInputStream,
                                               String properties, String originalFilename, String comment)
        ...
        Bitstream oldBitstream = null;
        
        for(Bitstream bs : bitStreams) {
        	if(bitstream.getID().equals(bs.getID())) {
        		break;
        	}
        	if(bitstream.getName() != null && bs.getName().equalsIgnoreCase(bitstream.getName())) {
        		bitstream.setName(context, bs.getName());
        		oldBitstream = bs;
        	}
        }
        
        if(oldBitstream != null) {
        	VersionBitstream vb = vbService.createVersion(context, ePerson, bundle, oldBitstream, bitstream, comment, name);
        	bitstream.setSequenceID(-1);
        	bitstream.setVersionBitstream(vb);
        } else {
        	vbService.createFirstVersion(context, ePerson, bundle, bitstream, comment ,name);
        }
