...
public class StatelessLoginFilter extends AbstractAuthenticationProcessingFilter {
    ...
    protected void unsuccessfulAuthentication(HttpServletRequest request,
        ...
        int statusError = HttpServletResponse.SC_INTERNAL_SERVER_ERROR;
        String error = "";
        if (failed instanceof CredentialsExpiredException) {
        	statusError = HttpServletResponse.SC_FORBIDDEN;
        	error = "Password has expired.";
        } else if (failed instanceof LockedException) {
        	statusError = 429;        	
        	String remainTime = request.getAttribute("remainingTime").toString();
        	error = "Account is locked. Please try again after "+remainTime;
        } else if (failed instanceof BadCredentialsException) {
        	statusError = HttpServletResponse.SC_UNAUTHORIZED;
        	error = "Invalid username or password.";
        }  else if (failed instanceof DisabledException) {
        	statusError = HttpServletResponse.SC_FORBIDDEN ;
        	error = "User Disabled From Login." ;
        }else {
        	statusError = HttpServletResponse.SC_BAD_REQUEST;
        	error = "Authentication failed.";
        }
        response.sendError(statusError, error);
