...
public class RestDSpaceRunnableHandler implements DSpaceRunnableHandler {
     ...
     */
    public RestDSpaceRunnableHandler(EPerson ePerson, String scriptName, List<DSpaceCommandLineParameter> parameters,
                                     final Set<Group> specialGroups, List<MultipartFile> files) throws SQLException {
        Context context = new Context();
        ...
        try {
            ...
            if (files != null && !files.isEmpty()) {
            	...
            	if (fileName != null && fileName.contains(".")) {
                    batchName = fileName.substring(0, fileName.lastIndexOf("."));
                }
            	List<Process> processList = processService.findByBatchName(context, batchName);
            	if (checkBatchUploadStatus(processList)) {
            		this.scriptName = null;
            		throw new SQLException("Duplicate batch found.");
            	}
            }
    ...
    private boolean checkBatchUploadStatus(List<Process> processList) {
        if (processList == null || processList.isEmpty()) {
            return false;
        }

        Set<ProcessStatus> validStatuses = EnumSet.of(
        ...
        for (Process process : processList) {
            ...
            if ("import".equalsIgnoreCase(process.getName()) &&
                process.getProcessStatus() != null &&
                validStatuses.contains(process.getProcessStatus())) {
                return true;
            }
        }

        return false; // No matching conditions found
    }
    ...
    public void writeFilestream(Context context, String fileName, InputStream inputStream, String type)
        ...
        // TODO update Batch-import Details here
    }

    @Override
    public void writeFilestream(Context context, String fileName, InputStream inputStream, String type, List<Long> counts)
        throws IOException, SQLException, AuthorizeException {
    	if (processId == null) {
    		throw new SQLException("Duplicate batch found.");
    	}
        Process process = processService.find(context, processId);
        String batchName = fileName.substring(0, fileName.lastIndexOf("."));
        if(process.getBatchName() == null || process.getBatchName().isBlank()) {
        	process.setBatchName(batchName);
        }

        processService.appendFile(context, process, inputStream, type, fileName);
        
        batchHistory(context,fileName, counts);
        batchDetails(context,fileName, counts);
        uploadDetails(context,fileName, counts);
    }
    
    public void batchDetails(Context context, String fileName, List<Long> counts)
    		throws IOException, SQLException, AuthorizeException 
    {
    	String batchName = fileName.lastIndexOf(".") == -1 ? fileName : fileName.substring(0, fileName.lastIndexOf("."));
    	List<Batchdetails> batchList = batchdetailsService.findAllByBatchName(context, batchName);
    	log.info("BatchDetails before creation"+ batchList);
		if (batchList != null && !batchList.isEmpty()) {
			Batchdetails batchdetails = batchList.get(0);
			String state = batchdetails.getState();
			if (!state.equalsIgnoreCase("claimed") && !state.equalsIgnoreCase("approved") && !state.equalsIgnoreCase("uploaded")) {
			...
			batchdetailsService.create(context, fileName, counts);
		}
    	return;
    }
    
    public void batchHistory(Context context, String fileName, List<Long> counts)
    		throws IOException, SQLException, AuthorizeException 
    {
        batchhistoryService.create(context,fileName,context.getCurrentUser(), counts);
    	return;
    }
    
    public void uploadDetails(Context context, String fileName, List<Long> counts)
    		throws IOException, SQLException, AuthorizeException 
    {
    	String batchName = fileName.lastIndexOf(".") == -1 ? fileName : fileName.substring(0, fileName.lastIndexOf("."));
    	if(uploaddetailsService.findByBatchNameAndStatus(context, batchName, "success").isEmpty() && uploaddetailsService.findByBatchNameAndStatus(context, batchName, "uploading").isEmpty())
    	{
    		uploaddetailsService.create(context,fileName,context.getCurrentUser(), counts);
    	}
     	return;
    }
