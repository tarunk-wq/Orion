...
public abstract class JWTTokenHandler {
     ...
     * @throws SQLException
     */
    public EPerson parseEPersonFromToken(String token, HttpServletRequest request, Context context, boolean isShortLivedToken)
        ...
        //uncaching the eperson object from hibernate cache
        context.uncacheEntity(ePerson);
        
        EPerson ePersonFound = ePersonService.find(context, ePerson.getID());

        if(!checkIfValidToken(ePersonFound,request.getRemoteAddr(),token, isShortLivedToken)) {        	
        	return null;
        }
        // As long as the JWT is valid, parse all claims and return the EPerson
        if (isValidToken(request, signedJWT, jwtClaimsSet, ePersonFound)) {
        	
        	if (!isShortLivedToken) {
        		extendTokenDuration(context, ePersonFound);
        	}
    ...
    private boolean checkIfValidToken(EPerson eperson, String currentRequestIPAddress, String token, boolean isShortLivedToken) {
    	String epersonToken = isShortLivedToken ? eperson.getShortLivedToken() :eperson.getSessionJWTToken(); 
        if(epersonToken == null || !epersonToken.equals(token)) {
        		return false;
        }
        if(!eperson.getEpersonIPAddress().equals(currentRequestIPAddress)) {
        	return false;        	
        }
        return true;
    }
    
    private void extendTokenDuration(Context context, EPerson eperson) {
    	Instant expiration = eperson.getTokenExpirationTime();
    	if (expiration == null) {
            return;
        }
    	
    	long timeLeftMillis = expiration.toEpochMilli() - System.currentTimeMillis();
        final long tenMinutesMillis = 10 * 60 * 1000;
        
        if (timeLeftMillis >= tenMinutesMillis) {
            return;
        }
        
        
        try {
        	eperson.setTokenExpirationTime(
                Instant.now().plusMillis(getExpirationPeriod())
            );
            ...
            log.error("Error extending token expiration", e);
        }
    }
     ...
     * @param isShortLivedToken 
     * @return string version of signed JWT
     * @throws JOSEException passed through.
     * @throws SQLException passed through.
     */
    public String createTokenForEPerson(Context context, HttpServletRequest request, Instant previousLoginDate, boolean isShortLivedToken)
        ...
        try {			
        	if(isShortLivedToken) {
        		ePerson.setShortLivedToken(token);
        	}else {
        		ePerson.setSessionJWTToken(token);
        		ePerson.setTokenExpirationTime(Instant.now().plusMillis(getExpirationPeriod()));
        	}
        	ePerson.setEpersonIPAddress(request.getRemoteAddr());
        	ePersonService.update(context, ePerson);
        } catch (Exception e) {
        	log.error("Error in setting token expiration period for eperson",e);
        }
    ...
    public void invalidateToken(String token, HttpServletRequest request, Context context) throws Exception {
        if (StringUtils.isNotBlank(token)) {
            ...
            EPerson ePerson = parseEPersonFromToken(token, request, context, false);
            if (ePerson != null) {
